...
for e in [-0.5,-0.6,-0.7,-0.8,-0.9]: #später ncoh die anderen e ergänzen!
    Rg2plot = np.array([])
    cplot = np.array([])
    ...
    plt.semilogx(cplot,Rg2plot, label=r"$\epsilon={}$".format(float(e)),
             color=colors[k],marker=markers[k],basex=2,ms=10)
    plt.semilogx(cplot,Rg2plot,alpha=0.6, color=colors[k],basex=2)
